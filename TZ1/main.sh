...
read -rp "Введите путь к входной директории: " input_dir
...
read -rp "Введите путь к выходной директории: " output_dir
...
# Проверяем, есть ли права на чтение у выходной директории
if [[ ! -r "$output_dir" ]]; then
    echo "Ошибка: нет прав на чтение выходной директории $output_dir."
    exit 1
fi

# Проверяем, есть ли права на запись в выходную директорию
if [[ ! -w "$output_dir" ]]; then
    echo "Ошибка: нет прав на запись в выходную директорию $output_dir."
    exit 1
fi
